...
def get_genome(organism=None,
    ...
    if organism:
        ...
        if isinstance(ids, str):
            ...
            ids = '%2C'.join([ids.replace("GCA", "GCF") for g in ids])
        ...
        query = f"https://api.ncbi.nlm.nih.gov/datasets/v2/genome/bioproject/{bioproject}/dataset_report?filters.reference_only={reference_only}"
    ...
    if n > len(metadata_json['reports']): warnings.warn(f"{n=} higher then number of avaible genomes, {len(metadata_json['reports'])} will be downoaded")

    reports = metadata_json['reports']
    if organism:
        reports = reports[:n]

    os.makedirs(output_dir, exist_ok=True)
    for r in reports:
        query, filename = _build_query(_get_ids(r), "genomic")
        print(query)
...
def _build_query(id_dict, mode):
    ...
    protein_filename = f"{id_dict['GCF']}_{id_dict['Assembly_Name']}_{mode}.fna.gz"
